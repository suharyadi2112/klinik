 ...
 $(function () {  
    ...
    $(document).on('click', '.addReligion', function () {
        Swal.fire({
          ...
          preConfirm: (data) => {
            if (data) {
                return fetch(`{{ Route('StoreReligion') }}`,{
                    method: 'POST',
                    headers: {
                      'Content-Type': 'application/json',
                      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    body: JSON.stringify({nameReligion: data})
                }).then(response => {
    ...
    //delete
    $(document).on('click', '.delReligion', function () {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
              if (id) {
                return fetch('{{route("DelReligion", ":id")}}'.replace(":id", id),{
                    method: 'DELETE',
                    headers: {
                      'Content-Type': 'application/json',
                      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    ...
    //update
    $(document).on('click', '.upReligion', function () {
        var id = $(this).attr('data-id')
        ...
        Swal.fire({
          ...
          preConfirm: (data) => {
            if (data) {
                return fetch('{{route("PutIReligion", ":id")}}'.replace(":id", id),{
                    method: 'POST',
                    headers: {
                      'Content-Type': 'application/json',
                      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    body: JSON.stringify({NewUpdateReligion: data})
                }).then(response => {
